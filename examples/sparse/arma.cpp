...
int main(){
	...
	arma::SpMat<int>M(16,30);
	for(int i = 0, j=dist(gen); i<M.n_elem; i += j)
	   	M[i] = j, j = dist(gen);
	//std::cout << M;
	std::cout <<"size: " << M.n_cols << "x" << M.n_rows 
	...
	h5::gr_t gr2 = h5::gopen(fd,"sparse-multi-file.gzip");
	::arma::Row<double> values = h5::read<::arma::Row<double>>(gr, "values" );
