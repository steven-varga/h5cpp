...
int main(){
	{ // CREATE - WRITE
		...
		hid_t fd = h5::create("blaze.h5"); 	// and a file
			h5::write(fd,"blaze/M",M); 		// save dataset
		h5::close(fd); 						// close fd
	}
	{ // CREATE - READ 
		hid_t fd = h5::open("blaze.h5", H5F_ACC_RDWR); 				// you can have multiple fd open with H5F_ACC_RDONLY, but single write
	...
	{ // READ
		Matrix<double> M = h5::read<Matrix<double>>("blaze.h5","blaze/M"); 	// read entire dataset back with a single read
		std::cout << M<<std::endl;
	}
	{
		// PARTIAL READ
		hid_t fd = h5::create("blaze-partial.h5");
		Colvec<float> data(5);  for( int i=0; i<5; i++ ) data[i] = i;
		hid_t ds = h5::create<float>(fd,"/blaze/data",{5,5,1,2});
