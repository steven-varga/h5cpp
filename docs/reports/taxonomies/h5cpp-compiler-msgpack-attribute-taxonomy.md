...
**Status:** Implemented on `vargalabs/h5cpp-compiler#34-messagepack-backend`
**Repos:** `vargalabs/h5cpp-compiler` (annotation reader + MessagePack descriptor emitter, issue #34)
**Scope:** MessagePack backend C++ attribute vocabulary. Defines every `[[msgpack::...]]` attribute the compiler recognizes, its emission semantics, and its C++26 migration path.
**Companions:**
...
- `vargalabs/h5cpp-compiler#34` — `src/consumer_msgpack.hpp`, `src/h5_attr_translator.hpp`, `src/h5cpp.cpp`
