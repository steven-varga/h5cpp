...
**Status:** Implemented on `vargalabs/h5cpp-compiler#32-h5-attribute-support`
**Repos:** `vargalabs/h5cpp` (library-side scatter/gather, issue #258) · `vargalabs/h5cpp-compiler` (annotation reader + HDF5 backend emitter, issue #32)
**Scope:** HDF5-backend C++ attribute vocabulary. Defines every `[[h5::...]]` attribute the compiler recognizes, its emission semantics, and its C++26 migration path.
**Companions:**
- `tasks/h5cpp-compiler-scatter-gather-design.md` — tier-1..4 model, `row_t` / VLEN design, library-side dispatch
- `tasks/h5cpp-compiler-multi-backend-architecture.md` — multi-backend roof; `h5::proto::*` stays in its own namespace there
...
1. **`h5::compress` algorithm expansion.** Today only `"gzip"` is supported. Adding `"shuffle"`, `"szip"`, `"zstd"` requires mapping strings to `H5Pset_shuffle`, `H5Pset_szip`, future `H5Pset_filter` calls. Should the argument shape stay `(algo, level)` or become `(algo, param1, param2, ...)` for algorithms with multiple parameters?
2. **`h5::on_missing` default for `gather`.** Today `"create"` is the default for `scatter` (create dataset if absent). For `gather`, `"create"` doesn't make sense — you can't read from a dataset that doesn't exist. The current implementation treats `"create"` as "open the dataset" for `gather`, which will fail at runtime if the dataset is missing. Should `gather` have a separate default (e.g. `"error"`)?
3. **`h5::serialize_full` and `h5::ignore` interaction.** If a user marks a field with both `[[h5::ignore]]` and the struct has `[[h5::serialize_full]]`, the field is skipped twice (redundant but harmless). Should the compiler emit a warning for redundant attributes?
4. **`h5::chunk` multi-dimensional support.** Today `h5::chunk(N)` produces a 1D chunk because scatter/gather datasets are 1D. If future emission paths support multi-dimensional datasets, `h5::chunk(N, M, ...)` should emit `hsize_t dims[] = {N, M, ...}` and `H5Pset_chunk(dcpl, rank, dims)`. The consumer already reads all integer args; only the producer limits itself to the first value.
5. **`h5::doc` at field level.** Today `h5::doc` is only wired at class level (emitted as a comment before the generated namespace or `register_struct`). Field-level `h5::doc` would require emitting comments before each `H5Tinsert` call or storing doc strings as HDF5 attributes on the compound type members. HDF5 has no native per-member doc mechanism, so this is likely future work.
6. **Bounded buffers in structural-type sketches.** §6 used `std::array<char, 64>` for `h5::name` to satisfy structural-type constraints. 64 chars is enough for almost every name in practice. Final size before C++26 implementation.

---

## Sources

- `tasks/h5cpp-compiler-scatter-gather-design.md` (workspace) — tier model, `row_t` / VLEN design, library-side dispatch
- `tasks/h5cpp-compiler-multi-backend-architecture.md` (workspace) — multi-backend roof and the `h5::proto::*` sub-namespace
- `tasks/h5cpp-compiler-pb-attribute-taxonomy.md` (workspace) — protobuf attribute vocabulary (same universal words, `pb::` namespace)
- `vargalabs/h5cpp#258` — library-side `h5::has_scatter<T>`, `H5CPP_REGISTER_SCATTER(T)`, `h5::detail::write_one_row` / `read_one_row`
- `vargalabs/h5cpp-compiler#32` — `src/h5_attr_translator.hpp`, `src/h5_attr_reader.hpp`, `src/consumer.hpp`, `src/producer_h5.hpp`
- [HDF5 User Guide — Dataset Chunking](https://support.hdfgroup.org/documentation/hdf5/latest/group___h5_d.html)
- [HDF5 User Guide — Compression Filters](https://support.hdfgroup.org/documentation/hdf5/latest/group___h5_p.html)
- [P2996R13 — Reflection for C++26](https://wg21.link/p2996)
- [P3394R4 — Annotations for Reflection](https://wg21.link/p3394)

## Related examples

- [`examples/attributes/attributes.cpp`](../../../examples/attributes/attributes.cpp) — h5cpp-compiler attribute demo (canonical reference for the h5 backend's attribute model)
