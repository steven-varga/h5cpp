...
**Scope:** The non-intrusive `operator<<` overload family in `h5cpp/H5Uall.hpp` that streams STL-shaped containers — and their elements, recursively — to `std::ostream`.
**Target audience:** Developers debugging h5cpp round-trips, writing examples, or producing one-line summaries of in-memory data.
**Last updated:** 2026-05-27 (branch `prototype`, working tree)
...
- [`examples/cout/cout.cpp`](../../../examples/cout/cout.cpp) — handle pretty-printing demo
- [`examples/stl/strings.cpp`](../../../examples/stl/strings.cpp) — STL strings round-trip (canonical text)
- [`examples/pprint/pprint.cpp`](../../../examples/pprint/pprint.cpp) — STL container pretty-print
